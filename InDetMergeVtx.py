...
def execute(nSamples,
    ...
    if (models["vertex_lstm"] == 1):
        ...
        lstm_vtx_hist, lstm_vtx_scores, lstm_vtx_x_test, lstm_vtx_y_test, lstm_vtx_mod = utl.train_model(lstm_vtx_mod, feat_vtx_3D, lab_vtx_3D, epochs, nEp)
        lstm_vtx_result, lstm_vtx_result1D, lstm_vtx_result_roc, lstm_vtx_probs, lstm_vtx_probs1D = utl.prediction(lstm_vtx_mod, lstm_vtx_x_test)  # modificato, controllare se funziona!!!!
    ...
    if (models["conc_best"] == 1):
        ...
        start = time.time()
        new_conc_model, new_conc_hist, new_conc_scores, new_conc_x_test_vtx,\
         new_conc_y_test_vtx, new_conc_x_test_trk, new_conc_y_test_trk = mod.new_concat_model(
            feat_vtx, lab_vtx, feat_trk_new_conc_3D, 
            lab_trk_new_conc_3D, epochs, nVtxFeats, nTrackFeats, loss)
        stop = time.time()
